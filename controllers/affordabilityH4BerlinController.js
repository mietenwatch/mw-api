...
exports.index = (req, res, next) => {
  ...
  model.db.all(
    "SELECT mw_data.geo_ortsteil_id AS ot_id, mw_data.geo_ortsteil AS ot_name, SUM(CASE WHEN cst_gesamtmiete <= ? THEN 1 ELSE 0 END) AS ot_affordableFlats, count(*) as ot_totalFlats FROM mw_data WHERE " +
      roomsQuery +
    ...
    (err, rows) => {
      if (typeof rows == "undefined" || rows.length == 0 || err) {
        ...
        const berlinData = [];
        model.db.get(
          "SELECT COUNT(*) AS berlin_totalFlats, SUM(CASE WHEN cst_gesamtmiete <= ? THEN 1 ELSE 0 END) AS berlin_affordableFlats FROM mw_data WHERE " +
            roomsQuery,
            [income],
          (err, row) => {
            if (err) {
